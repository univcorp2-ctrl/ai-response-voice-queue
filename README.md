...
Cloudflare Pages の本番 URL はリポジトリ作成後に README へ反映されます。デモでは回答キュー、優先順位、操作中の待機、読み上げをブラウザ上で体験できます。

### Chrome 拡張として使う

1. GitHub Actions の `ai-response-voice-queue-extension` artifact を取得して展開します。
2. Chrome で `chrome://extensions` を開きます。
3. 「デベロッパー モード」を有効にします。
4. 「パッケージ化されていない拡張機能を読み込む」から展開した `dist` フォルダを選びます。
5. ChatGPT または Claude を開き、拡張機能ポップアップで「自動読み上げ」を有効にします。
